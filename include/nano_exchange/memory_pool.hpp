...
class MemoryPool
{
    ...
    // - when the position in empty, holds the size_t index of the next free slot
    ...
    explicit MemoryPool(size_t capacity) : pool_(capacity), nextFreeIndex_(0)
    {
        ...
        for(size_t i = 0; i < capacity - 1; i++)
            pool_[i].nextFreeIndex_ = i + 1;

        pool_[capacity - 1].nextFreeIndex_ = capacity;  // capacity is the "full" signal
    }
    ...
    T* allocate()
    {
        ...
        nextFreeIndex_ = pool_[temp].nextFreeIndex_;
    ...
    void deallocate(T* pointer)
    {
        ...
        if(std::less<Slot*>{}(slot_pointer, pool.data()) || std::greater_equal{}(slot_pointer, pool.data() + pool.size()))
            return;

        size_t difference = slot_pointer - pool.data();
        pool[difference].nextFreeIndex = nextFreeIndex;
        nextFreeIndex = difference;
    }
